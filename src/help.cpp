...
std::vector<Page> Help::pages = {
   /* #pragma region Introduction
    ...
    Page::fromFunction("Infinite series","infinite_sum","Returns the sum of the lambda as `n` goes to infinity. Example: `infinite_sum(n=>1/n!) = 1/0! + 1/1! + 1/2! + 1/3! + 1/4! + 1/5! ... = 2.71828183`. If a starting index is required, it can be the second argument. This can be used to execute the taylor series or test for convergence. The function will, however, return whatever result it got after `100000` iterations. Otherwise, the function will stop when the desired precision is reached, whether it is an ?arb? number or a regular one.",{"sum"},"https://en.wikipedia.org/wiki/Series_(mathematics)#Examples_of_numerical_series"),
    Page::fromFunction("Definite integral","dint","Returns the definite integral of `func` from `a` to `b`. The definite integral is the area under the curve. This function uses Simpson's method to approximate the area, and successively increases the number of intervals until a desired accuracy is reached. If `a` and `b` are swapped, it will be the same answer but negative. Examples: `dint(x=>x^2,0,1) = 0.333333333`, `dint(x=>sin(x),0,pi/2) = 1`, `dint(x=>sqrt(x)+3x,1,4) = 27.16666667`,",{"calculus"},"https://math.libretexts.org/Courses/Mount_Royal_University/MATH_2200%3A_Calculus_for_Scientists_II/2%3A_Techniques_of_Integration/2.5%3A_Numerical_Integration_-_Midpoint%2C_Trapezoid%2C_Simpson's_rule"),
    #pragma endregion
    ...
    Page::fromFunction("Every","every","Returns `1` or `0` depending on whether all elements in the vector return true when passed through `func`. Example: `every(<1,2,3>,x=>(x>0))` returns `1` because every element in the vector is greater than zero.",{"all","vector","lambda"}),
